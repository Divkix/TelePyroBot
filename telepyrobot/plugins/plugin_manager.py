...
async def install_plugin(client, message):
    if len(message.command) == 1 and message.reply_to_message.document:
        if message.reply_to_message.document.file_name.split(".")[-1] != "py":
            ...
            return
        plugin_loc = (
            f"/app/telepyrobot/plugins/{message.reply_to_message.document.file_name}"
        )
        await message.edit("`Installing plugin...`")
        if os.path.exists(plugin_loc):
            await message.edit(
                f"`Plugin {message.reply_to_message.document.file_name} already exists!`"
            )
            return
        try:
            plugin_dl_loc = await client.download_media(
                message=message.reply_to_message, file_name=plugin_loc
            )
            if plugin_dl_loc:
                await message.edit(
                    f"**Installed plugin:** {message.reply_to_message.document.file_name}"
                )
